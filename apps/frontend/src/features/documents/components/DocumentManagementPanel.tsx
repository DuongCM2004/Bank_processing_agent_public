...
import { useQueueCaseProcessingMutation, useUploadCaseDocumentMutation } from "@/features/cases/hooks";
...
export function DocumentManagementPanel({
  ...
  const queueProcessingMutation = useQueueCaseProcessingMutation(caseId);
  ...
  const extractionSectionRef = useRef<HTMLDivElement | null>(null);
  ...
  // Backend exposes audit events only at `/cases/{caseId}/audit-events`. We scope
  // by `resource_type=document` and then filter by the selected document's id
  // on the client so the panel shows events for the current document only.
  const auditQuery = useCaseAuditEventsQuery({ caseId, resourceType: "document" });
  const documentAuditItems = (auditQuery.data?.items ?? []).filter(
    (event) => !selectedDocumentId || event.entity_id === selectedDocumentId,
  );
  ...
  useEffect(() => {
    if (!queueProcessingMutation.isPending) {
      return;
    }

    setExtractionProgress((current) => (current === null ? 5 : current));
    const interval = window.setInterval(() => {
      setExtractionProgress((current) => {
        ...
        const step = current < 50 ? 4 : current < 80 ? 2 : 0.8;
  ...
  async function handleUpload(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      handleClearFile();
      setFeedback("Document uploaded. Extract information is available once the document list refreshes.");
    } catch (error) {
  ...
  async function handleExtractInformation() {
    ...
    if (documents.length === 0) {
      setFeedback("Upload at least one document before starting extraction.");
      return;
    }

    try {
      const response = await queueProcessingMutation.mutateAsync({
        actor_id: actorId.trim() || "document-intake-ui",
        reason_code: "manual_llm_extraction_requested",
        run_synchronously: true,
      });
      if (selectedDocumentId) {
        await Promise.all([documentStatusQuery.refetch(), extractionQuery.refetch(), auditQuery.refetch()]);
      }
      setExtractionProgress(100);
      window.setTimeout(() => setExtractionProgress(null), 900);
      window.setTimeout(() => {
        extractionSectionRef.current?.scrollIntoView({ behavior: "smooth", block: "start" });
      }, 150);
      setFeedback(
        `AI extraction finished. Case status: ${response.status}. Processed documents: ${
          response.processed_document_count ?? documents.length
        }.`,
      );
    } catch (error) {
      setExtractionProgress(null);
      setFeedback(error instanceof Error ? error.message : "Information extraction could not be started.");
    }
  }
  ...
  function handleManualExtraction() {
    ...
    if (documents.length === 0) {
      setFeedback("Upload at least one document before starting manual extraction.");
      return;
    }
    ...
    setFeedback("Manual extraction mode: fill the reviewed fields below, then click Save edits.");
  }
    ...
    >
      <div className="grid gap-5 xl:grid-cols-[minmax(320px,0.9fr)_minmax(0,1.1fr)]">
        <form onSubmit={handleUpload} className="rounded-2xl border border-line p-4">
          ...
          <div className="mt-4 grid gap-3">
            ...
            >
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate">Choose a PNG or JPEG image of an identity card (national ID, passport, driver license).</p>
                </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <Button
                type="button"
                disabled={documents.length === 0 || queueProcessingMutation.isPending}
                onClick={() => void handleExtractInformation()}
              >
                {queueProcessingMutation.isPending ? "AI extracting..." : "AI extraction"}
              </Button>
              <Button
                type="button"
                variant="secondary"
                ...
                <div className="flex items-center justify-between text-xs text-slate">
                  <span>
                    {extractionProgress >= 100
                      ? "LLM LangGraph extraction complete"
                      : "LLM LangGraph extraction in progress..."}
                  </span>
                  <span className="font-mono">{Math.round(extractionProgress)}%</span>
                </div>
        ...
        <section className="rounded-2xl border border-line p-4">
          ...
          {selectedDocument ? (
            <div ref={extractionSectionRef} className="mt-5 rounded-2xl border border-line bg-white p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
              ...
              <div className="mt-5 border-t border-line pt-4">
                ...
                {documentAuditItems.length === 0 ? (
                  <p className="mt-2 text-sm text-slate">No audit events found for this document.</p>
                ) : (
                  <div className="mt-3 space-y-2">
                    {documentAuditItems.slice(0, 5).map((event) => (
